...
typedef enum
{
	...
	geoERR_COMMON_NO_MEMORY,					//内存不足
	geoERR_COMMON_DATATYPE,						//数据类型无效
	geoERR_COMMON_DATACHANGE_FAILED,			
	geoERR_COMMON_PARAMETER_INVALID,			//参数无效
	geoERR_COMMON_FILE_NOT_EXIST,				//文件不存在
	geoERR_COMMON_FILE_FAILED,					//文件不完整
	geoERR_COMMON_CHECK_FAILED,					//校验失败
...
typedef enum
{
	...
	geoERR_TAG_DATAFILE_NOTEXIST,					//标签数据配置文件不存在
	geoERR_TAG_DATAFILE_VERSION,					//标签数据配置文件版本不对
	geoERR_TAG_COUNT_OVERFLOW,						//标签数量已经超出范围
	geoERR_TAG_NOT_EXIST,							//标签不存在
	geoERR_TAG_DELETE_ROOT,							//标签根节点删除错误
	geoERR_TAG_HAVE_DELETED,						//标签已经被删除
	geoERR_TAG_NAME_REDUPLICATE,					//标签名重复添加
	geoERR_TAG_NAME_NOT_EXIST,						//标签名不存在
	geoERR_TAG_NAMEHASH_FAILTURE,					//标签名映射错误
	geoERR_TAG_CANNOT_SETPROP,						//设置属性失败
	geoERR_TAG_CANNOT_GETPROP,						//读取属性失败
	geoERR_TAG_ADD,									//标签添加失败
