...
__inline void ClearValueList(geoVariantList* pValueList)
{
	...
	memset(pValueList,0,sizeof(geoVariantList));
}

__inline void FreeValueList(geoVariantList** ppValueList)
{
...
__inline geoAPIStatus NewValueList(geoVariantList** ppValueList)
{
	return NewAndClear((geoVoid**)ppValueList,(geoUInt32)sizeof(geoVariantList));
}

__inline geoAPIStatus NewValueListEx(geoVariantList **ppValueList,geoUInt32 nCount)
{
	geoAPIStatus nRet = geoRET_SUCC;
	...
	RET_ERR(NewAndClear((geoVoid**)ppValueList,sizeof(geoVariantList)));
	geoVariantList* pValueList = *ppValueList;
	pValueList->ValueCount = nCount;
	nRet = NewAndClear((geoVoid**)&(pValueList->ValueList),sizeof(geoVariantList)*nCount);
	if(geoFail(nRet))
	{
