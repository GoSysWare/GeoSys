...
{
	if (a== 0) return;
	if (a->left!= 0) elibmem(a->left);
	if (a->right!= 0) elibmem(a->right);
	delete a;
}
...
CExpression::arbore CExpression::identificator()
{
	...
	if (m_definitie[pozitie]=='\0') result	=  0;
...
double CExpression::vexp ( arbore a )
{
	...
	if (a->operatie==0) {error_code=10;return 0;}
	switch(a->operatie){
	...
	case 160 : return tm.tm_year;
	case 161 : return tm.tm_mon;
	case 162 : return tm.tm_mday;
	case 163 : return tm.tm_hour;
	case 164 : return tm.tm_min;
	case 165 : return tm.tm_sec;
	case 166 : return std::max(vexp(a->left),vexp(a->right));
	case 167 : return std::min(vexp(a->left),vexp(a->right));
	case 168 : return random(vexp(a->left));
