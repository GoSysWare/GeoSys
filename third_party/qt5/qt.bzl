...
def qt_cc_library(name, srcs, hdrs, copts = [], uis = [], res = [], normal_hdrs = [], deps = None, **kwargs):
    for hItem in hdrs:
        ...
        if grep -q Q_OBJECT $(location %s); then \
            /opt/Qt5.14.2/5.14.2/gcc_64/bin/moc $(location %s) -o $@ -f'%s'; \
        else \
            echo '' > $@ ; \
    ...
    for uitem in uis:
        ...
        native.genrule(
            ...
            cmd = "/opt/Qt5.14.2/5.14.2/gcc_64/bin/uic $(locations %s) -o $@" % uitem,
        )
    ...
    for ritem in res:
        ...
        native.genrule(
            ...
            cmd = "/opt/Qt5.14.2/5.14.2/gcc_64/bin/rcc --name res --output $(OUTS) $(location %s)" % ritem,
        )
