...
geoInt32 CRecTableCol::TC_AddRecord(geoVariant *pVal)
{
    ...
    if(info_.Datatype != pVal->vType)
    {
        return geoRET_FAIL;
    }
	if (pVal->vType == vTypeString ||
		pVal->vType == vTypeAString ||
		pVal->vType == vTypeWString ||
		pVal->vType == vTypeBlob)
	{
		info_.CurSize++;
...
geoInt32 CRecTableCol::TC_QueryAdapterEqual(geoVariant *nval, geoUInt32 rangeCount, geoRecnoList **recnoList)
{
	...
	if (nval->vType == vTypeAString||
		nval->vType == vTypeString ||
		nval->vType == vTypeWString ||
		nval->vType == vTypeBlob)
	{
		//todo:remmber
...
geoInt32 CRecTableCol::TC_QueryAdapterScope(geoVariant *nvBegin, geoVariant *nvEnd, geoUInt32 rangeCount, POUT geoRecnoList **recnoList)
{
	...
	if (nvBegin->vType != info_.Datatype || nvEnd->vType != info_.Datatype)
	{
		return geoRET_FAIL;
	}
