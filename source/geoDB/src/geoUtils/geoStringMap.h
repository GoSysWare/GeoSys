...
public:
	...
	~GeoStringMap(geoVoid);
	geoAPIStatus bind(geoChar *name, geoUInt32 &id); //名称绑定	
	geoAPIStatus find(geoChar *name, geoUInt32 &id); //查找	
	geoAPIStatus find(geoChar *name) //查找	
	{
		geoUInt32 nTempId;
		return find(name, nTempId);
	}
	geoAPIStatus unbind(geoChar *name, geoUInt32 id = 0); //解除绑定
	geoAPIStatus init(); //初始化
