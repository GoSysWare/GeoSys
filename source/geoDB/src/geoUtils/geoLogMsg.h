...
do {\
	int __geo_error = GeoLog::last_error_adapter(); \
	GeoLog *geo = GeoLog::instance(); \
	geo->set_flags(GeoLog::OSTREAM);    \
	geo->clr_flags(GeoLog::STDERR | GeoLog::LOGGER);    \
	geo->conditional_set(__FILE__, __LINE__, __geo_error); \
	geo->log(LM_DEBUG,X); \
} while (0)
//系统日志输出log
...
do {\
	int __geo_error = GeoLog::last_error_adapter(); \
	GeoLog *geo = GeoLog::instance(); \
	geo->set_flags(GeoLog::SYSLOG);    \
	geo->clr_flags(GeoLog::STDERR | GeoLog::LOGGER |GeoLog::OSTREAM);    \
	geo->conditional_set(__FILE__, __LINE__, __geo_error); \
	geo->log(LM_DEBUG,X); \
} while (0)
